<!DOCTYPE html>
<html lang="ru" dir="ltr">
  <head>
    <meta charset="utf-8">
    <title>Заказы</title>
    <link rel="stylesheet" href="assets/css/style_main.css">
    <link rel="stylesheet" href="assets/css/style_orders.css">
    <link rel="preconnect" href="https://fonts.gstatic.com">
    <link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@100;200;300;400;500;600;700;800;900&display=swap" rel="stylesheet">
  </head>
  <body>

    <?php
      $connection = mysqli_connect('localhost', 'root', 'root', 'crmdb');
      if ($connection == false) {
        echo 'Не удалось подключиться к БД';
        echo my_sql_connect_error();
        exit();
      }
    ?>

<!-- Sprite -->

    <svg style="display: none;">

      <symbol id="home" viewBox="0 0 463.937 463.937">
        <g>
      		<path d="M460.414,216.518l-85.6-73.1c0.1-0.5,0.2-1,0.3-1.6v-95.6c-0.1-5.7-4.6-10.3-10.3-10.5h-47.8c-5.5,0-10,4.9-10,10.5v39.6
      			l-68.5-58.4c-3.7-3.2-9.2-3.2-13,0l-221.9,189c-4.3,3.5-4.8,9.8-1.3,14.1s9.8,4.8,14.1,1.3c0.1-0.1,0.1-0.1,0.2-0.2l215.4-183.4
      			l77.1,65.7l46.1,39.2l92.3,78.6c4.2,3.6,10.5,3.1,14.1-1.1C465.114,226.418,464.614,220.118,460.414,216.518z M355.014,126.518
      			l-28-23.6v-47.1h28V126.518z"/>
      	</g>
        <g>
      		<path d="M416.314,236.718l-28.1-24l-149.7-127.9c-3.7-3.2-9.2-3.2-13,0l-149.7,127.9l-28.1,24c-4.2,3.6-4.7,9.9-1.1,14.1
      			c3.5,4.2,9.7,4.7,13.8,1.2l0.1-0.1l12.5-10.8v187.5c0.1,5.6,4.7,10.2,10.3,10.3h297.3c5.6-0.1,10.2-4.6,10.3-10.3v-187.5
      			l12.5,10.8c1.8,1.5,4,2.4,6.4,2.4c2.9,0,5.7-1.3,7.6-3.5C421.114,246.518,420.514,240.218,416.314,236.718z M272.014,418.818h-80
      			v-108h80V418.818z M292.014,418.818v-117.7c0-5.5-4.1-10.3-9.6-10.3h-100.8c-5.5,0-9.6,4.8-9.6,10.3v117.7h-79v-194.8l139-118.4
      			l139,118.4v194.8H292.014z"/>
      	</g>
      </symbol>

      <symbol id="clients" viewBox="0 0 512 512">
        <g>
          <g>
            <path d="m256 207c47.972 0 87-39.028 87-87s-39.028-87-87-87-87 39.028-87 87 39.028 87 87 87zm0-144c31.43 0 57 25.57 57 57s-25.57 57-57 57-57-25.57-57-57 25.57-57 57-57z"/>
            <path d="m432 207c30.327 0 55-24.673 55-55s-24.673-55-55-55-55 24.673-55 55 24.673 55 55 55zm0-80c13.785 0 25 11.215 25 25s-11.215 25-25 25-25-11.215-25-25 11.215-25 25-25z"/>
            <path d="m444.1 241h-23.2c-27.339 0-50.939 16.152-61.693 39.352-22.141-24.17-53.944-39.352-89.228-39.352h-27.957c-35.284 0-67.087 15.182-89.228 39.352-10.755-23.2-34.355-39.352-61.694-39.352h-23.2c-37.44 0-67.9 30.276-67.9 67.49v109.21c0 16.156 13.194 29.3 29.412 29.3h91.727c1.538 17.9 16.59 32 34.883 32h199.957c18.292 0 33.344-14.1 34.883-32h90.679c16.796 0 30.46-13.61 30.46-30.34v-108.17c-.001-37.214-30.461-67.49-67.901-67.49zm-414.1 67.49c0-20.672 17.002-37.49 37.9-37.49h23.2c20.898 0 37.9 16.818 37.9 37.49v10.271c-10.087 26.264-8 42.004-8 98.239h-91zm331 135.489c0 2.769-2.252 5.021-5.021 5.021h-199.958c-2.769 0-5.021-2.253-5.021-5.021v-81.957c0-50.19 40.832-91.022 91.021-91.022h27.957c50.19 0 91.022 40.832 91.022 91.021zm121-27.319c0 .517 5.592.34-91 .34 0-56.651 2.071-72.018-8-98.239v-10.271c0-20.672 17.002-37.49 37.9-37.49h23.2c20.898 0 37.9 16.818 37.9 37.49z"/>
            <path d="m80 207c30.327 0 55-24.673 55-55s-24.673-55-55-55-55 24.673-55 55 24.673 55 55 55zm0-80c13.785 0 25 11.215 25 25s-11.215 25-25 25-25-11.215-25-25 11.215-25 25-25z"/>
          </g>
        </g>
      </symbol>

      <symbol id="worker" viewBox="0 0 512.001 512.001">
        <g>
          <path d="M504.501,421.9c-4.143,0-7.5,3.358-7.5,7.5v50.113c0,0.696-0.566,1.263-1.263,1.263H467.24V371.199
      			c0-4.142-3.357-7.5-7.5-7.5s-7.5,3.358-7.5,7.5v109.578H397.17l14.905-16.397c2.302-2.533,3.387-6.13,2.904-9.623
      			c-0.011-0.075-0.022-0.149-0.035-0.224l-16.755-98.996l3.083-4.641l0.803,0.57c2.749,1.953,6.255,2.665,9.647,1.818
      			c3.331-0.833,6.122-3.074,7.656-6.146v-0.001l17.049-34.171l38.945,15.083c1.537,0.668,21.625,9.879,21.625,33.845v32.504
      			c0.001,4.143,3.358,7.501,7.501,7.501c4.143,0,7.5-3.358,7.5-7.5v-32.504c0-34.61-29.685-47.211-30.948-47.731
      			c-0.048-0.02-0.095-0.039-0.143-0.057l-66.317-25.684v-13.66c10.352-8.328,17.622-20.331,19.805-34.014
      			c5.163-0.648,9.953-2.715,13.928-6.134c5.7-4.902,8.971-12.035,8.971-19.568c0-6.013-2.087-11.767-5.824-16.337v-20.238
      			c0-39.606-32.222-71.827-71.826-71.827c-17.781,0-34.061,6.508-46.618,17.25c-0.162-5.747-2.219-11.223-5.804-15.607V88.051
      			c0-39.606-32.222-71.827-71.826-71.827c-39.606,0-71.827,32.222-71.827,71.827v20.765c-3.605,4.308-5.622,9.502-5.8,15.015
      			c-8.012-6.847-17.601-12.002-28.288-14.816c-4.012-1.056-8.109,1.337-9.164,5.343s1.337,8.109,5.343,9.164
      			c24.913,6.56,42.313,29.157,42.313,54.949v11.217c-0.324-0.067-1.364-0.246-1.404-0.252c-0.102-2.117-0.638-3.41-1.792-5.07
      			c-1.92-2.759-5.368-6.196-16.458-18.116c-3.952-4.245-10.344-5.024-15.196-1.852c-18.995,12.408-42.072,18.732-63.488,17.475
      			c-6.976-0.409-11.333,2.656-13.19,7.679c-0.166,0.027-2.021,0.401-2.124,0.426v-11.507c0-24.869,16.618-47.242,40.411-54.409
      			c3.966-1.194,6.213-5.378,5.019-9.345c-1.195-3.967-5.385-6.214-9.346-5.018c-14.47,4.358-27.477,13.432-36.622,25.548
      			c-9.463,12.534-14.464,27.481-14.464,43.225v20.766c-3.778,4.514-5.821,10-5.821,15.809c0,12.84,9.46,24.038,22.938,25.708
      			c2.208,13.363,9.458,26.178,20.9,34.891v12.781l-66.243,25.682c-0.047,0.018-0.094,0.037-0.141,0.056
      			C29.686,314.684,0,327.284,0,361.894v117.619c0,8.969,7.296,16.264,16.265,16.264h230.971c3.228,0,6.231-0.957,8.765-2.585
      			c2.532,1.628,5.536,2.585,8.765,2.585h230.972c8.968,0,16.264-7.295,16.264-16.264v-50.113
      			C512.001,425.258,508.644,421.9,504.501,421.9z M75.702,225.258c-4.397-1.596-7.206-5.663-7.206-10.213
      			c0-4.413,3.2-8.039,7.206-9.76V225.258z M422.379,307.525l-14.722,29.506c-0.002-0.002-16.353-11.615-16.585-11.78
      			c7.054-8.708,3.74-4.617,18.408-22.721L422.379,307.525z M380.23,314.803l-19.397-23.943v-7.195
      			c6.305,2.18,13.097,3.186,19.49,3.188c0.001,0,0.003,0,0.004,0h0.001c6.61-0.001,13.302-1.256,19.262-3.499v7.552L380.23,314.803z
      			 M389.036,342.206l-2.759,4.153h-12.091l-2.759-4.153l8.805-6.253L389.036,342.206z M255.981,224.381l-19.397-23.943v-7.195
      			c6.28,2.171,13.065,3.186,19.49,3.188c0.001,0,0.003,0,0.004,0h0.001c6.598,0,13.289-1.251,19.262-3.498v7.551L255.981,224.381z
      			 M264.786,251.786l-2.759,4.153h-12.091l-2.759-4.153l8.805-6.253L264.786,251.786z M196.918,149.614
      			c1.226,0.334,2.486,0.561,3.765,0.719c2.208,13.363,9.458,26.178,20.9,34.891v12.781l-15.301,5.931
      			c-0.887-1.863-1.996-3.618-3.312-5.227v-20.237C202.97,168.294,200.826,158.51,196.918,149.614z M295.836,141.672
      			c0,21.953-17.908,39.756-39.757,39.756c-24.651,0-41.125-19.105-41.125-39.756V106.54c23.683,0.948,48.286-6.071,68.401-18.591
      			c6.763,7.252,10.787,11.465,12.482,13.421C295.836,120.043,295.836,127.253,295.836,141.672z M310.147,150.328
      			c1.272-0.16,2.526-0.388,3.745-0.721c-3.896,8.84-6.076,18.6-6.076,28.865v20.765c-1.153,1.377-2.145,2.844-2.966,4.383
      			l-14.508-5.618v-13.66C300.693,176.013,307.964,164.01,310.147,150.328z M226.705,212.12l18.412,22.726l-12.556,8.918
      			c-0.007,0.005-0.012,0.01-0.019,0.015l-3.985,2.831l-14.723-29.505L226.705,212.12z M273.243,226.904l11.987-14.796l12.899,4.995
      			l-14.721,29.507l-3.994-2.836c-0.004-0.003-0.008-0.007-0.012-0.01l-12.579-8.935L273.243,226.904z M277.023,260.475l0.803,0.57
      			c2.804,1.992,6.323,2.65,9.647,1.818c3.331-0.833,6.122-3.074,7.656-6.146v-0.001l12.658-25.369
      			c4.349,5.36,10.639,8.62,17.117,9.429c2.288,14.34,10.039,26.578,20.929,34.87v12.78l-63.764,24.721l-8.13-48.03L277.023,260.475z
      			 M350.954,302.541l18.412,22.726c-8.05,5.719-16.143,11.468-16.56,11.764l-14.723-29.505L350.954,302.541z M420.084,232.094
      			c0.002,21.417-17.391,39.756-39.756,39.756c-25.492,0-41.125-19.966-41.125-39.756v-35.132c23.514,0.939,48.149-5.987,68.4-18.591
      			c6.763,7.252,10.787,11.465,12.482,13.421C420.084,210.464,420.084,217.675,420.084,232.094z M442.294,215.046
      			c0,4.558-2.818,8.617-7.206,10.212v-20.403C439.624,206.507,442.294,210.652,442.294,215.046z M322.817,178.471
      			c0-31.334,25.493-56.826,56.826-56.826c31.334,0,56.825,25.493,56.825,56.826v11.217c-0.341-0.071-1.279-0.233-1.404-0.252
      			c-0.102-2.119-0.641-3.415-1.792-5.07c-1.971-2.831-5.381-6.209-16.458-18.116c-3.868-4.152-10.246-5.087-15.196-1.852
      			c-18.48,12.072-41.828,18.891-64.045,17.448c-6.241-0.409-10.762,2.647-12.633,7.707c-0.167,0.027-2.019,0.401-2.124,0.426
      			V178.471z M324.202,205.286v19.962c-4.41-1.59-7.206-5.668-7.206-10.203C316.995,210.632,320.193,207.008,324.202,205.286z
      			 M318.043,124.625c0,3.158-1.367,6.145-3.75,8.194c-1.023,0.88-2.189,1.556-3.456,2.017v-20.403
      			C315.322,116.061,318.043,120.184,318.043,124.625z M255.393,31.224c31.334,0,56.826,25.492,56.826,56.826v11.217
      			c-0.339-0.07-1.279-0.233-1.404-0.252c-0.111-2.297-0.773-3.709-2.124-5.536c-2.008-2.718-5.71-6.455-16.126-17.651
      			c-3.933-4.223-10.321-5.038-15.196-1.852c-19.516,12.75-43.191,18.955-64.65,17.418c-5.25-0.397-10.193,2.775-12.027,7.736
      			c-0.167,0.027-2.02,0.401-2.124,0.426V88.051h-0.001C198.567,56.716,224.06,31.224,255.393,31.224z M199.951,114.864v19.973
      			c-4.397-1.596-7.206-5.663-7.206-10.213C192.745,120.236,195.92,116.597,199.951,114.864z M186.587,204.855
      			c4.196,1.527,6.891,5.188,7.158,9.539c0.336,4.838-2.637,9.213-7.158,10.855V204.855z M185.892,240.774
      			c7.074-0.885,13.567-4.613,17.825-10.348l13.118,26.29c0,0,0,0,0.001,0.001c1.533,3.073,4.324,5.313,7.655,6.146
      			c3.293,0.823,6.816,0.193,9.647-1.818l0.803-0.57l3.083,4.641l-8.127,48.017l-63.804-24.711v-13.654h-0.001
      			C176.715,266.203,183.76,254.101,185.892,240.774z M131.73,314.803l-19.397-23.943v-7.195c6.28,2.171,13.066,3.186,19.491,3.187
      			c0.001,0,0.003,0,0.004,0c6.706,0,13.386-1.288,19.262-3.498v7.552L131.73,314.803z M140.536,342.206l-2.759,4.153h-12.091
      			l-2.759-4.153l8.805-6.253L140.536,342.206z M160.979,302.53l12.9,4.996l-14.722,29.505l-3.994-2.836
      			c-0.004-0.003-0.008-0.007-0.012-0.01l-12.579-8.934L160.979,302.53z M90.703,232.094v-35.132
      			c23.809,0.951,48.428-6.161,68.4-18.591c6.699,7.184,10.785,11.463,12.482,13.421c0,18.673,0,25.884,0,40.302
      			c0,21.813-17.785,39.756-39.757,39.756C107.365,271.85,90.703,252.963,90.703,232.094z M102.455,302.541l18.412,22.726
      			l-12.556,8.918c-0.007,0.005-0.012,0.01-0.019,0.015l-3.985,2.831l-14.723-29.505L102.455,302.541z M59.763,480.778V371.199
      			c0-4.142-3.357-7.5-7.5-7.5s-7.5,3.358-7.5,7.5v109.578H16.265c-0.698,0-1.264-0.567-1.264-1.263V361.894
      			c0-24.089,20.297-33.271,21.611-33.839l38.921-15.089l17.051,34.171c0,0,0,0,0.001,0.001c1.533,3.073,4.324,5.313,7.656,6.146
      			c3.293,0.823,6.816,0.193,9.647-1.818l0.803-0.57l3.083,4.641l-16.755,98.996c-0.013,0.074-0.024,0.149-0.035,0.224
      			c-0.482,3.491,0.603,7.088,2.906,9.625l14.903,16.395H59.763z M112.073,455.481l15.929-94.121h7.459l15.93,94.121l-19.659,21.627
      			L112.073,455.481z M248.499,479.513c0,0.697-0.566,1.264-1.263,1.264H218.74V371.199c0-4.142-3.357-7.5-7.5-7.5
      			c-4.143,0-7.5,3.358-7.5,7.5v109.578H148.67l14.905-16.397c2.302-2.533,3.387-6.13,2.904-9.623
      			c-0.011-0.075-0.022-0.149-0.035-0.224l-16.755-98.996l3.083-4.641l0.803,0.57c2.007,1.426,4.383,2.17,6.794,2.17
      			c4.399,0,8.496-2.465,10.51-6.498v-0.001l17.049-34.17l38.945,15.083c11.352,4.938,21.625,17.673,21.625,33.844V479.513z
      			 M256,334.64c-3.107-5.006-7.243-9.638-12.229-13.591l8.481-50.109h7.459l8.485,50.135C263.077,325.14,258.995,329.814,256,334.64
      			z M308.262,480.778L308.262,480.778V371.199c0-4.142-3.357-7.5-7.5-7.5s-7.5,3.358-7.5,7.5v109.578h-28.497
      			c-0.697,0-1.264-0.567-1.264-1.263V361.894c0-8.916,2.796-17.128,8.752-24.027c3.971-4.555,9.238-8.243,12.86-9.812l38.921-15.09
      			l17.051,34.172c0,0,0,0,0.001,0.001c1.533,3.073,4.324,5.313,7.655,6.146c3.293,0.823,6.816,0.193,9.647-1.818l0.803-0.57
      			l3.083,4.641l-16.755,98.996c-0.013,0.074-0.024,0.149-0.035,0.224c-0.482,3.491,0.603,7.088,2.906,9.625l14.903,16.395H308.262z
      			 M360.572,455.482l15.93-94.121h7.459l15.93,94.121l-19.66,21.627L360.572,455.482z"/>
        </g>
      </symbol>

      <symbol id="reports" viewBox="0 0 480.006 480.006">
        <g>
        	<g>
        		<rect x="40.003" y="48.006" width="104" height="16"/>
        	</g>
        </g>
        <g>
        	<g>
        		<rect x="40.003" y="80.006" width="104" height="16"/>
        	</g>
        </g>
        <g>
        	<g>
        		<rect x="40.003" y="112.006" width="72" height="16"/>
        	</g>
        </g>
        <g>
        	<g>
        		<rect x="40.003" y="144.006" width="72" height="16"/>
        	</g>
        </g>
        <g>
        	<g>
        		<rect x="0.003" y="464.006" width="480" height="16"/>
        	</g>
        </g>
        <g>
        	<g>
        		<path d="M463.163,68.43l-32-64c-1.973-3.954-6.776-5.56-10.73-3.587c-0.002,0.001-0.004,0.002-0.006,0.003l-64,32
        			c-3.954,1.975-5.559,6.782-3.584,10.736s6.782,5.559,10.736,3.584l46.352-23.16l-12.648,36.112
        			C341.69,218.446,198.501,329.522,31.323,344.006l1.36,16C206.16,344.978,354.737,229.701,412.395,65.398l12.912-36.896
        			l23.536,47.08c1.975,3.954,6.782,5.559,10.736,3.584C463.534,77.191,465.138,72.385,463.163,68.43z"/>
        	</g>
        </g>
        <g>
        	<g>
        		<path d="M96.003,384.006h-56c-4.418,0-8,3.582-8,8v48c0,4.418,3.582,8,8,8h56c4.418,0,8-3.582,8-8v-48
        			C104.003,387.588,100.421,384.006,96.003,384.006z M88.003,432.006h-40v-32h40V432.006z"/>
        	</g>
        </g>
        <g>
        	<g>
        		<path d="M184.003,360.006h-56c-4.418,0-8,3.582-8,8v72c0,4.418,3.582,8,8,8h56c4.418,0,8-3.582,8-8v-72
        			C192.003,363.588,188.421,360.006,184.003,360.006z M176.003,432.006h-40v-56h40V432.006z"/>
        	</g>
        </g>
        <g>
        	<g>
        		<path d="M272.003,320.006h-56c-4.418,0-8,3.582-8,8v112c0,4.418,3.582,8,8,8h56c4.418,0,8-3.582,8-8v-112
        			C280.003,323.588,276.421,320.006,272.003,320.006z M264.003,432.006h-40v-96h40V432.006z"/>
        	</g>
        </g>
        <g>
        	<g>
        		<path d="M360.003,264.006h-56c-4.418,0-8,3.582-8,8v168c0,4.418,3.582,8,8,8h56c4.418,0,8-3.582,8-8v-168
        			C368.003,267.588,364.421,264.006,360.003,264.006z M352.003,432.006h-40v-152h40V432.006z"/>
        	</g>
        </g>
        <g>
        	<g>
        		<path d="M448.003,152.006h-56c-4.418,0-8,3.582-8,8v280c0,4.418,3.582,8,8,8h56c4.418,0,8-3.582,8-8v-280
        			C456.003,155.588,452.421,152.006,448.003,152.006z M440.003,432.006h-40v-264h40V432.006z"/>
        	</g>
        </g>
      </symbol>

      <symbol id="orders" viewBox="0 0 438.891 438.891">
        <g>
    			<path d="M347.968,57.503h-39.706V39.74c0-5.747-6.269-8.359-12.016-8.359h-30.824c-7.314-20.898-25.6-31.347-46.498-31.347
    				c-20.668-0.777-39.467,11.896-46.498,31.347h-30.302c-5.747,0-11.494,2.612-11.494,8.359v17.763H90.923
    				c-23.53,0.251-42.78,18.813-43.886,42.318v299.363c0,22.988,20.898,39.706,43.886,39.706h257.045
    				c22.988,0,43.886-16.718,43.886-39.706V99.822C390.748,76.316,371.498,57.754,347.968,57.503z M151.527,52.279h28.735
    				c5.016-0.612,9.045-4.428,9.927-9.404c3.094-13.474,14.915-23.146,28.735-23.51c13.692,0.415,25.335,10.117,28.212,23.51
    				c0.937,5.148,5.232,9.013,10.449,9.404h29.78v41.796H151.527V52.279z M370.956,399.185c0,11.494-11.494,18.808-22.988,18.808
    				H90.923c-11.494,0-22.988-7.314-22.988-18.808V99.822c1.066-11.964,10.978-21.201,22.988-21.42h39.706v26.645
    				c0.552,5.854,5.622,10.233,11.494,9.927h154.122c5.98,0.327,11.209-3.992,12.016-9.927V78.401h39.706
    				c12.009,0.22,21.922,9.456,22.988,21.42V399.185z"/>
    			<path d="M179.217,233.569c-3.919-4.131-10.425-4.364-14.629-0.522l-33.437,31.869l-14.106-14.629
    				c-3.919-4.131-10.425-4.363-14.629-0.522c-4.047,4.24-4.047,10.911,0,15.151l21.42,21.943c1.854,2.076,4.532,3.224,7.314,3.135
    				c2.756-0.039,5.385-1.166,7.314-3.135l40.751-38.661c4.04-3.706,4.31-9.986,0.603-14.025
    				C179.628,233.962,179.427,233.761,179.217,233.569z"/>
    			<path d="M329.16,256.034H208.997c-5.771,0-10.449,4.678-10.449,10.449s4.678,10.449,10.449,10.449H329.16
    				c5.771,0,10.449-4.678,10.449-10.449S334.931,256.034,329.16,256.034z"/>
    			<path d="M179.217,149.977c-3.919-4.131-10.425-4.364-14.629-0.522l-33.437,31.869l-14.106-14.629
    				c-3.919-4.131-10.425-4.364-14.629-0.522c-4.047,4.24-4.047,10.911,0,15.151l21.42,21.943c1.854,2.076,4.532,3.224,7.314,3.135
    				c2.756-0.039,5.385-1.166,7.314-3.135l40.751-38.661c4.04-3.706,4.31-9.986,0.603-14.025
    				C179.628,150.37,179.427,150.169,179.217,149.977z"/>
    			<path d="M329.16,172.442H208.997c-5.771,0-10.449,4.678-10.449,10.449s4.678,10.449,10.449,10.449H329.16
    				c5.771,0,10.449-4.678,10.449-10.449S334.931,172.442,329.16,172.442z"/>
    			<path d="M179.217,317.16c-3.919-4.131-10.425-4.363-14.629-0.522l-33.437,31.869l-14.106-14.629
    				c-3.919-4.131-10.425-4.363-14.629-0.522c-4.047,4.24-4.047,10.911,0,15.151l21.42,21.943c1.854,2.076,4.532,3.224,7.314,3.135
    				c2.756-0.039,5.385-1.166,7.314-3.135l40.751-38.661c4.04-3.706,4.31-9.986,0.603-14.025
    				C179.628,317.554,179.427,317.353,179.217,317.16z"/>
    			<path d="M329.16,339.626H208.997c-5.771,0-10.449,4.678-10.449,10.449s4.678,10.449,10.449,10.449H329.16
    				c5.771,0,10.449-4.678,10.449-10.449S334.931,339.626,329.16,339.626z"/>
    		</g>
      </symbol>

      <symbol id="reviews" viewBox="0 -107 512 512">
        <g>
          <path d="m461.574219 34.949219c-31.066407-22.539063-72.058594-34.949219-115.429688-34.949219-43.363281 0-84.351562 12.410156-115.414062 34.949219-32.515625 23.589843-50.421875 55.367187-50.421875 89.480469 0 31.847656 16.050781 62.144531 45.195312 85.3125 26.894532 21.378906 62.8125 34.867187 101.519532 38.183593 41.773437 30.378907 103.0625 47.527344 107.636718 48.773438 2.257813.636719 4.601563.960937 6.957032.960937 9.546874 0 18.316406-5.195312 22.847656-13.496094 5.832031-10.566406 3.65625-23.65625-5.320313-31.863281-11.390625-10.347656-17.464843-18.46875-20.699219-24.285156 20.296876-10.097656 37.402344-23.464844 49.921876-39.101563 15.460937-19.304687 23.632812-41.605468 23.632812-64.484374 0-34.113282-17.90625-65.890626-50.425781-89.480469zm-59.9375 131.78125h-110.964844c-8.289063 0-15.007813-6.71875-15.007813-15.007813 0-8.289062 6.71875-15.007812 15.007813-15.007812h110.964844c8.289062 0 15.007812 6.71875 15.007812 15.007812 0 8.289063-6.71875 15.007813-15.007812 15.007813zm0-50.246094h-110.964844c-8.289063 0-15.007813-6.71875-15.007813-15.007813 0-8.289062 6.71875-15.011718 15.007813-15.011718h110.964844c8.289062 0 15.007812 6.722656 15.007812 15.011718 0 8.289063-6.71875 15.007813-15.007812 15.007813zm0 0"/><path d="m150.433594 122.105469c0-23.8125 6.71875-46.746094 19.476562-67.628907-10.722656-1.996093-21.796875-3.035156-33.046875-3.035156-35.640625 0-69.355469 10.222656-94.941406 28.78125-27.03125 19.613282-41.921875 46.109375-41.921875 74.617188 0 19.109375 6.796875 37.710937 19.648438 53.804687 9.804687 12.269531 22.984374 22.84375 38.570312 31.023438-2.714844 4.152343-7.195312 9.628906-14.609375 16.363281-8.210937 7.503906-10.207031 19.527344-4.882813 29.179688 4.195313 7.679687 12.246094 12.449218 21.011719 12.449218 2.175781 0 4.332031-.296875 6.417969-.886718 4.179688-1.144532 53.210938-14.949219 87.195312-39.371094 23.253907-2.085938 44.96875-8.550782 63.46875-18.617188-2.597656-1.867187-5.144531-3.78125-7.625-5.753906-37.894531-30.121094-58.761718-69.515625-58.761718-110.925781zm0 0"/>
    		</g>
      </symbol>

      <symbol id="catalog" viewBox="0 0 512 512">
        <g>
          <path d="m363.328 0h-180.036c-4.142 0-7.5 3.358-7.5 7.5s3.358 7.5 7.5 7.5h160.17l-200.045 63.999c-10.162 2.441-17.242 11.468-17.242 21.87v317.758h-21.292c-4.134 0-7.497-3.363-7.497-7.497v-388.633c0-4.134 3.363-7.497 7.497-7.497h43.41c4.142 0 7.5-3.358 7.5-7.5s-3.358-7.5-7.5-7.5h-43.41c-12.405 0-22.498 10.092-22.498 22.497v388.634c0 12.405 10.092 22.497 22.497 22.497h21.292v55.875c0 12.405 10.092 22.497 22.497 22.497h258.446c12.405 0 22.497-10.092 22.497-22.497v-26.859c0-4.142-3.358-7.5-7.5-7.5s-7.5 3.358-7.5 7.5v26.859c0 4.134-3.363 7.497-7.497 7.497h-258.445c-4.134 0-7.497-3.363-7.497-7.497v-63.375c0-10.845 0-311.552 0-325.258 0-4.101 3.385-7.497 7.497-7.497h258.446c4.134 0 7.497 3.363 7.497 7.497v326.775c0 4.142 3.358 7.5 7.5 7.5s7.5-3.358 7.5-7.5v-326.776c0-12.405-10.092-22.497-22.497-22.497h-21.292v-55.875c-.001-12.327-9.998-22.497-22.498-22.497zm-168.726 78.373 176.208-56.373c.01.162.016.327.016.497v55.875h-176.224z"/><path d="m269.344 128.873c0-7.995-6.505-14.5-14.5-14.5h-78.169c-7.995 0-14.5 6.505-14.5 14.5v72.461c0 7.995 6.505 14.5 14.5 14.5h78.169c7.995 0 14.5-6.505 14.5-14.5zm-15 71.96h-77.169v-71.461h77.169z"/><path d="m254.844 374.539h-78.169c-7.995 0-14.5 6.505-14.5 14.5v72.461c0 7.995 6.505 14.5 14.5 14.5h78.169c7.995 0 14.5-6.505 14.5-14.5v-72.461c0-7.995-6.505-14.5-14.5-14.5zm-.5 86.461h-77.169v-71.461h77.169z"/><path d="m269.344 263c0-7.995-6.505-14.5-14.5-14.5h-78.169c-7.995 0-14.5 6.505-14.5 14.5v72.461c0 7.995 6.505 14.5 14.5 14.5h78.169c7.995 0 14.5-6.505 14.5-14.5zm-15 71.961h-77.169v-71.461h77.169z"/><path d="m386.115 143.43h-92.681c-4.142 0-7.5 3.358-7.5 7.5s3.358 7.5 7.5 7.5h92.681c4.142 0 7.5-3.358 7.5-7.5s-3.358-7.5-7.5-7.5z"/><path d="m293.434 186.776h34.667c4.142 0 7.5-3.358 7.5-7.5s-3.358-7.5-7.5-7.5h-34.667c-4.142 0-7.5 3.358-7.5 7.5s3.358 7.5 7.5 7.5z"/><path d="m386.115 277.557h-92.681c-4.142 0-7.5 3.358-7.5 7.5s3.358 7.5 7.5 7.5h92.681c4.142 0 7.5-3.358 7.5-7.5s-3.358-7.5-7.5-7.5z"/><path d="m293.434 320.904h34.667c4.142 0 7.5-3.358 7.5-7.5s-3.358-7.5-7.5-7.5h-34.667c-4.142 0-7.5 3.358-7.5 7.5s3.358 7.5 7.5 7.5z"/><path d="m393.615 411.096c0-4.142-3.358-7.5-7.5-7.5h-92.681c-4.142 0-7.5 3.358-7.5 7.5s3.358 7.5 7.5 7.5h92.681c4.142 0 7.5-3.358 7.5-7.5z"/><path d="m293.434 431.943c-4.142 0-7.5 3.358-7.5 7.5s3.358 7.5 7.5 7.5h34.667c4.142 0 7.5-3.358 7.5-7.5s-3.358-7.5-7.5-7.5z"/>
    		</g>
      </symbol>

      <symbol id="add" viewBox="0 0 512 512">
        <g>
          <path d="m256 0c-141.164062 0-256 114.835938-256 256s114.835938 256 256 256 256-114.835938 256-256-114.835938-256-256-256zm112 277.332031h-90.667969v90.667969c0 11.777344-9.554687 21.332031-21.332031 21.332031s-21.332031-9.554687-21.332031-21.332031v-90.667969h-90.667969c-11.777344 0-21.332031-9.554687-21.332031-21.332031s9.554687-21.332031 21.332031-21.332031h90.667969v-90.667969c0-11.777344 9.554687-21.332031 21.332031-21.332031s21.332031 9.554687 21.332031 21.332031v90.667969h90.667969c11.777344 0 21.332031 9.554687 21.332031 21.332031s-9.554687 21.332031-21.332031 21.332031zm0 0"/>
    		</g>
      </symbol>

      <symbol id="remove" viewBox="0 0 512 512">
        <g>
        <path d="m256 0c-141.164062 0-256 114.835938-256 256s114.835938 256 256 256 256-114.835938 256-256-114.835938-256-256-256zm94.273438 320.105469c8.339843 8.34375 8.339843 21.824219 0 30.167969-4.160157 4.160156-9.621094 6.25-15.085938 6.25-5.460938 0-10.921875-2.089844-15.082031-6.25l-64.105469-64.109376-64.105469 64.109376c-4.160156 4.160156-9.621093 6.25-15.082031 6.25-5.464844 0-10.925781-2.089844-15.085938-6.25-8.339843-8.34375-8.339843-21.824219 0-30.167969l64.109376-64.105469-64.109376-64.105469c-8.339843-8.34375-8.339843-21.824219 0-30.167969 8.34375-8.339843 21.824219-8.339843 30.167969 0l64.105469 64.109376 64.105469-64.109376c8.34375-8.339843 21.824219-8.339843 30.167969 0 8.339843 8.34375 8.339843 21.824219 0 30.167969l-64.109376 64.105469zm0 0"/>
    		</g>
      </symbol>

      <symbol id="edit" viewBox="0 0 490.337 490.337">
        <g>
      		<path d="M229.9,145.379l-47.5,47.5c-17.5,17.5-35.1,35-52.5,52.7c-4.1,4.2-7.2,9.8-8.4,15.3c-6.3,28.9-12.4,57.8-18.5,86.7
      			l-3.4,16c-1.6,7.8,0.5,15.6,5.8,20.9c4.1,4.1,9.8,6.4,15.8,6.4c1.7,0,3.4-0.2,5.1-0.5l17.6-3.7c28-5.9,56.1-11.9,84.1-17.7
      			c6.5-1.4,12-4.3,16.7-9c78.6-78.7,157.2-157.3,235.8-235.8c5.8-5.8,9-12.7,9.8-21.2c0.1-1.4,0-2.8-0.3-4.1c-0.5-2-0.9-4.1-1.4-6.1
      			c-1.1-5.1-2.3-10.9-4.7-16.5l0,0c-14.7-33.6-39.1-57.6-72.5-71.1c-6.7-2.7-13.8-3.6-20-4.4l-1.7-0.2c-9-1.1-17.2,1.9-24.3,9.1
      			C320.4,54.879,275.1,100.179,229.9,145.379z M386.4,24.679c0.2,0,0.3,0,0.5,0l1.7,0.2c5.2,0.6,10,1.2,13.8,2.8
      			c27.2,11,47.2,30.6,59.3,58.2c1.4,3.2,2.3,7.3,3.2,11.6c0.3,1.6,0.7,3.2,1,4.8c-0.4,1.8-1.1,3-2.5,4.3
      			c-78.7,78.5-157.3,157.2-235.9,235.8c-1.3,1.3-2.5,1.9-4.3,2.3c-28.1,5.9-56.1,11.8-84.2,17.7l-14.8,3.1l2.8-13.1
      			c6.1-28.8,12.2-57.7,18.4-86.5c0.2-0.9,1-2.3,1.9-3.3c17.4-17.6,34.8-35.1,52.3-52.5l47.5-47.5c45.3-45.3,90.6-90.6,135.8-136
      			C384.8,24.979,385.7,24.679,386.4,24.679z"/>
      		<path d="M38.9,109.379h174.6c6.8,0,12.3-5.5,12.3-12.3s-5.5-12.3-12.3-12.3H38.9c-21.5,0-38.9,17.5-38.9,38.9v327.4
      			c0,21.5,17.5,38.9,38.9,38.9h327.3c21.5,0,38.9-17.5,38.9-38.9v-167.5c0-6.8-5.5-12.3-12.3-12.3s-12.3,5.5-12.3,12.3v167.5
      			c0,7.9-6.5,14.4-14.4,14.4H38.9c-7.9,0-14.4-6.5-14.4-14.4v-327.3C24.5,115.879,31,109.379,38.9,109.379z"/>
      	</g>
      </symbol>

    </svg>

    <div class="wrapper">

    <!-- ----------------------header--------------------- -->

      <header class="header">
        <div class="intro">
          <a class="intro__title" href="./index.html">
            <div class="intro__uptitle">CRM</div>
            <div class="intro__suptitle__block">
                <img class="intro__img" src="assets/images/orders/logo.png" alt="">AUTO
            </div>
          </a>
          <button class="burger" type="button" id="navToggle">
            <span class="burger__item">Menu</span>
          </button>
        </div>
      </header>

      <div class="content">
        <nav class="nav">

          <!--<div class="nav__title">Главное меню</div>-->

          <a class="nav__item" href="./menu.php">
            <svg class="nav__icon">
              <use xlink:href="#home"></use>
            </svg>
            <div class="nav__link">Главная</div>
          </a>

          <?php

          $position = $_COOKIE['role'];
                          
          if ($position != "user") {
            echo 
            '<a class="nav__item" href="./clients.php">
              <svg class="nav__icon">
                <use xlink:href="#clients"></use>
              </svg>
              <div class="nav__link">Клиенты</div>
            </a>';
          } 

          ?>

          <?php

            $position = $_COOKIE['role'];
                            
            if ($position == "Менеджер по продажам" || $position == "Администратор") {
              echo 
              '<a class="nav__item" href="./reports.php">
                <svg class="nav__icon">
                  <use xlink:href="#reports"></use>
                </svg>
                <div class="nav__link">Отчеты</div>
              </a>';
            } 

          ?>

          <?php

          $position = $_COOKIE['role'];
                          
          if ($position != "user") {
            echo 
            '<a class="nav__item" href="./workers.php">
            <svg class="nav__icon">
              <use xlink:href="#worker"></use>
            </svg>
            <div class="nav__link">Сотрудники</div>
          </a>';
          } 

          ?>

          <a class="nav__item" href="./reviews.php">
            <svg class="nav__icon">
              <use xlink:href="#reviews"></use>
            </svg>
            <div class="nav__link">Отзывы</div>
          </a>

          <a class="nav__item" href="./catalog.php">
            <svg class="nav__icon">
              <use xlink:href="#catalog"></use>
            </svg>
            <div class="nav__link">Каталог</div>
          </a>

        </nav>

        <div class="data">
          <div class="data__title">Заказы</div>
          <div class="data__instrument">
            <div class="data__instrument__left">
              <button class="data__btn btn--active" type="button" name="button">Все</button>
              <button class="data__btn btn--disable" type="button" name="button">Новые</button>
            </div>
            <div class="data__instrument__right">
              <input class="data__search" type="search" name="" value="" placeholder="Поиск">
              
              <?php

                $position = $_COOKIE['role'];
                
                if ($position == "Менеджер по продажам" || $position == "Администратор") {
                 echo 
                  '<div class="data__edit">
                      <svg class="edit__icon">
                        <use xlink:href="#edit"></use>
                      </svg>
                    </div>';
                }

                else {
                  echo 
                    '<div class="data__edit visually-hidden">
                      <svg class="edit__icon visually-hidden">
                        <use xlink:href="#edit"></use>
                      </svg>
                    </div>';
                }

              ?>


            </div>
          </div>
          <div class="data__content">
            <table class="data__table">
              <tr class="table__head"><th>Заказ №</th><th>Статус</th><th>Дата</th><th>Менеджер</th><th>Описание заказа</th><th>Клиент</th><th>Итого</th></tr>
              <tr class="table__firstrow"><th>Заказ №</th><th>Статус</th><th>Дата</th><th>Менеджер</th><th>Описание заказа</th><th>Клиент</th><th>Итого</th></tr>

              <?php

                $position = $_COOKIE['role'];

                //echo '<div>' . $_COOKIE['worker'] . '</div>';

                if ($position == "Менеджер по продажам" || $position == "Администратор") {
                  
                  $orders = mysqli_query($connection, "SELECT * FROM crm_order JOIN worker ON crm_order.worker_id = worker.worker_id JOIN user ON crm_order.user_id = user.user_id");

                  while (($ord = mysqli_fetch_assoc($orders))) {
                    echo
                      '<tr class="row"><td id="order_id">' . $ord['order_id'] .
                      '</td><td id="order_status"><select name="' . $ord['order_status'] . '" class="status"><option value="new">Новый</option><option value="processing">В обработке</option><option value="producting">В производстве</option><option value="send">Ожидает отправки</option><option value="complete">Выполнен</option></select></td><td>'. $ord['order_date'] .
                      '</td><td id="worker_id">' . $ord['worker_fn'] . ' ' . $ord['worker_sn'] .
                      '</td><td id="order_desc">' . $ord['order_desc'] .
                      '</td><td id="user_id">' . $ord['user_fn'] . ' ' . $ord['user_sn'] .
                      '</td><td id="order_price">' . $ord['order_price'] . ' RUB</td><td><svg class="remove__icon"><use xlink:href="#remove"></use></svg></td></tr>';
                  }
                }

                if ($position == "Менеджер по закупкам") {
                  
                  $orders = mysqli_query($connection, "SELECT * FROM crm_order JOIN worker ON crm_order.worker_id = worker.worker_id JOIN user ON crm_order.user_id = user.user_id WHERE order_status = 'В обработке' OR order_status = 'В производстве'");

                  while (($ord = mysqli_fetch_assoc($orders))) {
                    echo
                      '<tr class="row"><td id="order_id">' . $ord['order_id'] .
                      '</td><td id="order_status"><select name="' . $ord['order_status'] . '" class="status"><option value="processing">В обработке</option><option value="producting">В производстве</option></select></td><td>'. $ord['order_date'] .
                      '</td><td id="worker_id">' . $ord['worker_fn'] . ' ' . $ord['worker_sn'] .
                      '</td><td id="order_desc">' . $ord['order_desc'] .
                      '</td><td id="user_id">' . $ord['user_fn'] . ' ' . $ord['user_sn'] .
                      '</td><td id="order_price">' . $ord['order_price'] . ' RUB</td><td><svg class="remove__icon"><use xlink:href="#remove"></use></svg></td></tr>';
                  }
                }

                if ($position == "Грузчик") {
                  
                  $orders = mysqli_query($connection, "SELECT * FROM crm_order JOIN worker ON crm_order.worker_id = worker.worker_id JOIN user ON crm_order.user_id = user.user_id WHERE order_status = 'В производстве' OR order_status = 'Ожидает отправки'");

                  while (($ord = mysqli_fetch_assoc($orders))) {
                    echo
                      '<tr class="row"><td id="order_id">' . $ord['order_id'] .
                      '</td><td id="order_status"><select name="' . $ord['order_status'] . '" class="status"><option value="producting">В производстве</option><option value="send">Ожидает отправки</option></select></td><td>'. $ord['order_date'] .
                      '</td><td id="worker_id">' . $ord['worker_fn'] . ' ' . $ord['worker_sn'] .
                      '</td><td id="order_desc">' . $ord['order_desc'] .
                      '</td><td id="user_id">' . $ord['user_fn'] . ' ' . $ord['user_sn'] .
                      '</td><td id="order_price">' . $ord['order_price'] . ' RUB</td><td><svg class="remove__icon"><use xlink:href="#remove"></use></svg></td></tr>';
                  }
                }

                if ($position == "Курьер") {
                  
                  $orders = mysqli_query($connection, "SELECT * FROM crm_order JOIN worker ON crm_order.worker_id = worker.worker_id JOIN user ON crm_order.user_id = user.user_id WHERE order_status = 'Ожидает отправки' OR order_status = 'Выполнен'");

                  while (($ord = mysqli_fetch_assoc($orders))) {
                    echo
                      '<tr class="row"><td id="order_id">' . $ord['order_id'] .
                      '</td><td id="order_status"><select name="' . $ord['order_status'] . '" class="status"><option value="send">Ожидает отправки</option><option value="complete">Выполнен</option></select></td><td>'. $ord['order_date'] .
                      '</td><td id="worker_id">' . $ord['worker_fn'] . ' ' . $ord['worker_sn'] .
                      '</td><td id="order_desc">' . $ord['order_desc'] .
                      '</td><td id="user_id">' . $ord['user_fn'] . ' ' . $ord['user_sn'] .
                      '</td><td id="order_price">' . $ord['order_price'] . ' RUB</td><td><svg class="remove__icon"><use xlink:href="#remove"></use></svg></td></tr>';
                  }
                }

                if ($position == "user") {
                  
                  $user_id = $_COOKIE['user'];
                  
                  $orders = mysqli_query($connection, "SELECT * FROM crm_order JOIN worker ON crm_order.worker_id = worker.worker_id JOIN user ON crm_order.user_id = user.user_id WHERE crm_order.user_id = '$user_id'");

                  while (($ord = mysqli_fetch_assoc($orders))) {
                    echo
                      '<tr class="row"><td id="order_id">' . $ord['order_id'] .
                      '</td><td id="order_status"><select disabled name="' . $ord['order_status'] . '" class="status"><option value="new">Новый</option><option value="processing">В обработке</option><option value="producting">В производстве</option><option value="send">Ожидает отправки</option><option value="complete">Выполнен</option></select></td><td>'. $ord['order_date'] .
                      '</td><td id="worker_id">' . $ord['worker_fn'] . ' ' . $ord['worker_sn'] .
                      '</td><td id="order_desc">' . $ord['order_desc'] .
                      '</td><td id="user_id">' . $ord['user_fn'] . ' ' . $ord['user_sn'] .
                      '</td><td id="order_price">' . $ord['order_price'] . ' RUB</td><td><svg class="remove__icon"><use xlink:href="#remove"></use></svg></td></tr>';
                  }
                }

                
              ?>

            </table>
          </div>

          <?php

            $position = $_COOKIE['role'];
                
            if ($position == "Менеджер по продажам" || $position == "Администратор") {
              echo 
                '<div class="data__add">
                  <svg class="add__icon">
                    <use xlink:href="#add"></use>
                  </svg>
                </div>';
            }

            else {
              echo 
                '<div class="data__add visually-hidden">
                  <svg class="add__icon">
                    <use xlink:href="#add"></use>
                  </svg>
                </div>';
            }

          ?>

        </div>
      </div>

    <form class="popup popup--add">
      <div class="popup__title">Добавить заказ</div>
      <div class="popup__data">
        <input class="popup__input" type="number" name="order_id" value="" placeholder="№ заказа">
        <input class="popup__input" type="date" name="order_date" value="" placeholder="Дата">
        <input class="popup__input" type="text" name="worker_id" value="" placeholder="Номер менеджера">

        <div class="popup_autoparts">

          <div class="popup__item">
            <input class="popup__input input--id" type="number" name="part1" value="" placeholder="Номер запчасти">
            <input class="popup__input input--num" type="number" name="num1" value="" placeholder="">
          </div>

          <div class="part__add">
            <svg class="icon__popup">
              <use xlink:href="#add"></use>
            </svg>
          </div>

        </div>


        <input class="popup__input" type="text" name="user_id" value="" placeholder="Номер клиента">
      </div>

      <div class="popup__btns">
        <button class="popup__btn btn--cancel" type="button" name="button">Отменить</button>
        <button class="popup__btn btn--add" type="button" name="button">Добавить</button>
      </div>

    </form>


    <form class="popup popup--change" action="functions/orders/order_change.php">
      <div class="popup__title">Изменить данные</div>
      <div class="popup__data">
        <input class="popup__input" type="text" name="" value="" placeholder="">
      </div>
      <div class="popup__btns">
        <button class="popup__btn btn--cancel" type="button" name="button">Отменить</button>
        <button class="popup__btn btn--change" type="submit" name="button">Изменить</button>
      </div>
    </form>


    </div>

    <template id="autopart">
      <div class="popup__item">
        <input class="popup__input input--id" type="number" name="" value="" placeholder="Номер запчасти">
        <input class="popup__input input--num" type="number" name="" value="" placeholder="">
      </div>
    </template>

    <script src="js/main.js" type="text/javascript"></script>
    <script src="js/order.js" type="text/javascript"></script>

  </body>
</html>
